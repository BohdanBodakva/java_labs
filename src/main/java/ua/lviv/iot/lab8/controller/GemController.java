...
public class GemController {
    ...
    @GET
    @Path("/{id}")
    public ResponseEntity<GemEntity> showGemById(@PathParam(value = "id") Long id) {
        try {
            return new ResponseEntity<GemEntity>(gemService.getGemById(id), HttpStatus.OK);
        } catch (GemNotFoundException e) {
            return new ResponseEntity<GemEntity>(HttpStatus.NOT_FOUND);
